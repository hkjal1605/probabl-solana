...
export async function mountSolanaAdmin(
  ...
  await db.query(`CREATE TABLE IF NOT EXISTS solana_evidence(domain text NOT NULL,hash text NOT NULL,envelope jsonb NOT NULL,created_at timestamptz NOT NULL DEFAULT now(),PRIMARY KEY(domain,hash));
    ...
    CREATE TABLE IF NOT EXISTS solana_attachments(domain text NOT NULL,hash text NOT NULL,content bytea NOT NULL,PRIMARY KEY(domain,hash));`);
  ...
  const read = async (hash: string, query: Pool | PoolClient = db): Promise<View> => {
    ...
    const rows = await query.query(
      "SELECT envelope FROM solana_evidence WHERE domain=$1 AND hash=$2",
      [domain, hash],
    );
    const envelope = rows.rows[0]?.envelope as PacketEnvelope | undefined;
    if (!envelope) throw invalid("Evidence packet not found", 404);
    assertEvidenceIntegrity(envelope);
    const actions = await query.query(
    ...
    const select = (kind: string) =>
        actions.rows.filter((r) => r.kind === kind).map((r) => r.payload),
  ...
  const locked = async <T>(work: (tx: PoolClient) => Promise<T>) => {
    const tx = await db.connect();
    try {
      await tx.query("BEGIN");
      await tx.query("SELECT pg_advisory_xact_lock(hashtext($1))", ["evidence:" + domain]);
  ...
  const attachments = async (input: unknown, tx: PoolClient) => {
    ...
    for (const value of input) {
      ...
      await tx.query("INSERT INTO solana_attachments VALUES($1,$2,$3) ON CONFLICT DO NOTHING", [
        domain,
        contentHash,
        content,
      ]);
  ...
  const verifyAttachments = async (view: View, query: Pool | PoolClient = db) => {
    for (const reference of view.envelope.packet.attachments) {
      const r = await query.query(
        "SELECT content FROM solana_attachments WHERE domain=$1 AND hash=$2",
        [domain, reference.contentHash],
      );
      if (!r.rows[0]) throw invalid("Evidence attachment is missing", 409);
      assertAttachmentIntegrity(reference, r.rows[0].content);
    }
  };
  const assertMapping = (
    ...
    app.post(`/v1/admin/evidence/${kind}/prepare`, async (c) => {
      ...
      const hash = await locked(async (tx) => {
        ...
        await tx.query(
          ...
          [domain, value.packetHash, JSON.stringify(value)],
        );
  ...
  app.get("/v1/admin/evidence", async (c) => {
    ...
    return c.json({ packets: await Promise.all(rows.rows.map((r) => read(r.hash))) });
  });
  ...
  app.get("/v1/attachments/:hash", async (c) => {
    ...
    if (!row.rows[0] || attachmentContentHash(row.rows[0].content) !== hash)
      throw invalid("Attachment integrity failure", 409);
    return new Response(row.rows[0].content, {
      headers: {
