...
import { solanaQueries } from "../../../../packages/db/src/solana/queries";
import { indexedSnapshot } from "../../src/solana/chain/indexed-snapshot.ts";
...
test("API planning decodes a healthy persisted snapshot without any RPC", async () => {
  ...
  const data = await coder.accounts.encode("Config", {
  ...
  const result = await indexedSnapshot(solanaQueries(db as any), client, "domain");
...
test("missing or pre-migration snapshots fail closed without RPC fallback", async () => {
  ...
  for (const rows of [[], [{ accounts: {} }]])
    await expect(
      indexedSnapshot(solanaQueries({ query: async () => ({ rows }) } as any), client, "test"),
