...
import type { WalletKind } from "./admin-session";
...
export interface OperatorWallet {
  ...
  isPhantom?: boolean;
  isSolflare?: boolean;
  connect(options?: { onlyIfTrusted: true }): Promise<{ publicKey: PublicKey } | void>;
...
export interface WalletSources {
  phantom?: { solana?: OperatorWallet };
  solflare?: OperatorWallet;
  solana?: OperatorWallet;
}
export interface WalletHandle {
  kind: WalletKind;
  provider: OperatorWallet;
}
...
export async function connectOperatorWallet(handle: WalletHandle, silent: boolean) {
