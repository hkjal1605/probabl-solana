import { type Deployment, key } from "@conditional-stocks/solana-client";

export const ADMIN_SESSION_MS = 4 * 60 * 60 * 1000;
export const WALLET_STORAGE_KEY = "probabl:admin:wallet:v1";
export const SESSION_STORAGE_KEY = "probabl:admin:session:v1";
...
const ownerAddress = (value: unknown): value is string => {
  try {
    return typeof value === "string" && key(value).toBase58() === value;
  } catch {
    return false;
  }
};
...
export function createAdminSession(
  ...
  if (
    ...
    now < 0 ||
    !Number.isSafeInteger(now + ADMIN_SESSION_MS)
  )
    throw new Error("API returned an invalid Solana session");
  let expiresAt = now + ADMIN_SESSION_MS;
...
export function savedAdminSession(
  ...
  if (
    ...
    value.expiresAt > value.issuedAt + ADMIN_SESSION_MS
  )
    return null;
  return value as unknown as SavedAdminSession;
}
export const persistAdminSession = (storage: SessionStorage | null, session: SavedAdminSession) =>
...
let active: SavedAdminSession | null = null;
export function setRequestSession(value: SavedAdminSession | null) {
  active = value;
}
export function assertRequestSession(token: string, now = Date.now()) {
  if (
    ...
    throw new Error("Your four-hour operator session expired. Sign in again.");
}
