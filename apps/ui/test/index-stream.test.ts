...
} from "../src/services/index-stream";
...
test("healthy SSE disables only covered queries; failure restores polling", () => {
  const query = (kind: string) => ({
    queryKey: [kind],
    state: { status: "success", fetchFailureCount: 0 },
  });
...
test("wallet stream preserves external balances during an additive custody-field rollout", () => {
  const mint = PublicKey.unique().toBase58();
  const legacyBalance = {
    account: owner,
    token: mint,
    decimals: 6,
    canonicalBalance: "798602493340",
    blockNumber: "12",
    creditBalances: {},
  };
  const value = {
    ...
    blockNumber: "12",
    positions: [],
    balances: { [mint]: legacyBalance },
  };
  expect(parseStreamWallet(value, owner).balances[mint]).toMatchObject({
    canonicalBalance: "798602493340",
    vaultAvailable: "0",
    reserved: "0",
  });
  expect(() =>
    parseStreamWallet(
      {
        ...value,
        balances: { [mint]: { ...legacyBalance, vaultAvailable: "1" } },
      },
      owner,
    ),
  ).toThrow("Invalid streamed balance");
});
