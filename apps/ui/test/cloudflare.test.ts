...
describe("Workers runtime upstreams", () => {
  test("reads runtime settings lazily and retains Node local defaults", () => {
  ...
  test("streams POST bytes and forwards only permitted headers", async () => {
    ...
    const request = new NextRequest("https://ui.test/api/gateway/orders?mode=limit", {
      method: "POST",
      headers: {
  ...
  test("does not turn indexer proxy into a service endpoint or traversal tunnel", async () => {
    ...
    for (const path of [["internal"], ["health"], ["sql"], ["markets", ".."], ["markets", "%2f"]]) {
...
const valid = {
  ...
  NEXT_PUBLIC_CONDITIONAL_TOKENS_ADDRESS: "0x1111111111111111111111111111111111111111",
  NEXT_PUBLIC_EXCHANGE_ADDRESS: "0x2222222222222222222222222222222222222222",
  NEXT_PUBLIC_ATOMIC_ORDER_ROUTER_ADDRESS: "0x3333333333333333333333333333333333333333",
  NEXT_PUBLIC_PAYOUT_VAULT_ADDRESS: "0x4444444444444444444444444444444444444444",
  NEXT_PUBLIC_POSITION_ROUTER_ADDRESS: "0x5555555555555555555555555555555555555555",
};
describe("Workers deployment guard", () => {
  ...
  test("rejects local/placeholder URLs, insecure sockets, and invalid chain/address settings", () => {
    for (const [key, value] of [
      ...
      ["NEXT_PUBLIC_POLYMARKET_STREAM_URL", "wss://stream.probabl.com/already/path"],
