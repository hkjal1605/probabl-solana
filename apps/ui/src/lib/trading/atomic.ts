import { SolanaClient,parseOrder,parseAtomicPlan,orderId,envelope,verifyEnvelope,address,type Deployment } from "@conditional-stocks/solana-client";
export function atomicTransaction(input:{order:unknown;plan:unknown;account:string;config:Deployment;nowSeconds?:bigint}){
  ...
  return {orderHash:orderId(order,client.program),transaction:envelope([client.placement(order,plan)],client.program)};
}
