...
export function useProbabilityStream(
  ...
  useEffect(() => {
    ...
    const staleTimer = setInterval(
      () => setProbability((value) => expireProbability(value)),
      1000,
    );
    ...
    const connect = () => {
      ...
      try {
        socket = new WebSocket(probabilityStreamUrl(conditionId, base));
      } catch {
      ...
      socket.onclose = () => {
        if (stopped) return;
        attempt += 1;
        logger.warn("probability.stream.disconnected", {
          conditionId,
